...
export function marriageMarkerPosition(from: PositionedPerson, to: PositionedPerson, geometry: Geometry): { x: number; y: number } {
  if (from.y === to.y) return { x: (from.x + to.x + geometry.nodeWidth) / 2, y: from.y + geometry.nodeHeight / 2 };
...
export function edgePath(edge: TreeRelationship, from: PositionedPerson, to: PositionedPerson, geometry: Geometry): string {
  ...
  if (edge.type === 'spouse' && from.y === to.y) {
    const [left, right] = from.x < to.x ? [from, to] : [to, from];
    return `M ${left.x + geometry.nodeWidth} ${left.y + geometry.nodeHeight / 2} H ${right.x}`;
  }
  if (edge.type === 'parent') {
  ...
  // Other lateral links and spouses in different periods use the space above cards.
